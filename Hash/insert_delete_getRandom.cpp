...
public:
    ...
    bool remove(int val) {
        if (val2idx[val] == 0) return false;
        else {
            int idx = val2idx[val];
            nums[idx - 1] = nums.back();
            val2idx[nums[idx - 1]] = idx;
            val2idx.erase(val);
            nums.pop_back();
            return true;
        }
    ...
    int getRandom() {
        int i = randIdx(nums.size());
        return nums[i - 1];
    }
private:
    vector<int> nums;
    unordered_map<int, int> val2idx;
    int randIdx(int n)  // returns an index in range [1,len] inclusive
    {
        return 1 + rand() / ((RAND_MAX + 1u) / n);
    }   
};
