...
public:
    int findMaximizedCapital(int k, int w, vector<int>& profits, vector<int>& capital) {
        ...
        for (int i = 0; i < profits.size(); i++) pairs.push_back({capital[i], profits[i]});
        sort(pairs.begin(), pairs.end());
        int cap = w, cap_st = 0, cap_ix;
        priority_queue<int> mxHeap; // to efficiently retrieve running max of profits
        for (int ki = 0; ki < k; ki++) {
            cap_ix = cap_st;
            while (cap_ix < pairs.size() && pairs[cap_ix].first <= cap) mxHeap.push(pairs[cap_ix++].second);
            if (!mxHeap.empty()) {cap += mxHeap.top(); mxHeap.pop();}
            cap_st = cap_ix;
        }
        return cap;
    }
