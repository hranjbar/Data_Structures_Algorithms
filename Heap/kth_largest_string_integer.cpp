...
integer, "2" is the second-largest integer, and "1" is the 
...
public:
    string kthLargestNumber(vector<string>& nums, int k) {
        auto CustomCompare = [](string& a, string& b){
            if (a.length() != b.length()) return a.length() > b.length();
            else return a > b;
        };
        priority_queue<string, vector<string>, decltype(CustomCompare)> minHeap;
        for (string& num : nums) {
            minHeap.push(num);
