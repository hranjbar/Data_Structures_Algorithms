...
class Solution {
public:
    struct mycomp{  // needed to make priority min-queue of pair<int, int> based on second element
    ...
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        for (auto p : hash){
            // only keep k most frequent ones in heap
            if (minq.size() < k) minq.push(p);
            else {
                if (p.second > minq.top().second){
                    minq.pop(); 
                    minq.push(p);
                }
            }
