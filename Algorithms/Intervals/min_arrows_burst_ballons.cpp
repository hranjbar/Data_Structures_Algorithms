...
horizontal diameter stretches between xstart and 
xend. You do not know the exact y-coordinates of 
the balloons.
Arrows can be shot up directly vertically 
(in the positive y-direction) from different points
along the x-axis. A balloon with xstart and xend is 
burst by an arrow shot at x if xstart <= x <= xend. 
...
- Shoot an arrow at x = 6, bursting the 
balloons [2,8] and [1,6].
- Shoot an arrow at x = 11, bursting the 
...
Output: 2
Explanation: The balloons can be burst by 2 arrows:
- Shoot an arrow at x = 2, bursting the 
balloons [1,2] and [2,3].
- Shoot an arrow at x = 4, bursting the 
...
public:
    int findMinArrowShots(vector<vector<int>>& points) {
        ...
        for (int i=0; i<points.size()-1; i++) {
            if (points[i][1]<points[i+1][0]) ans++;
            else points[i+1][1] = min(points[i][1], points[i+1][1]);
        }
        return 1 + ans;
    }
};
