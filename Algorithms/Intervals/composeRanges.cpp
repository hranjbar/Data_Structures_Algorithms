...
public:
    vector<string> summaryRanges(vector<int>& nums) {
        ...
        intervals.push_back({nums[0], INT_MIN});
        for (int i=1; i<n; i++) {
            if (nums[i]>nums[i-1]+1) intervals.push_back({nums[i], INT_MIN});
            else intervals.back()[1] = nums[i];
        }
        vector<string> ans;
        for (auto& intv : intervals) {
            if (intv[0]<intv[1]) ans.push_back(to_string(intv[0])+"->"+to_string(intv[1]));
            else ans.push_back(to_string(intv[0]));
        }
        return ans;
    }
};
