...
The frog can only travel from point i to point i + 1 on the same lane if there is not an obstacle on 
the lane at point i + 1. To avoid obstacles, the frog can also perform a side jump to jump to 
another lane (even if they are not adjacent) at the same point if there is no obstacle on the new lane.
...
public:
    int minSideJumps(vector<int>& obstacles) {
        ...
        vector<vector<int>> dp(obstacles.size(), vector<int>(3, INT_MAX));
        dp.back() = {0, 0, 0};  // no obstacles on point n
        for (int i = n - 1; i >= 0; i--) {
            for (int cur : {1, 2, 3}) {
                if (cur != obstacles[i]) {
                    if (obstacles[i + 1] != cur) dp[i][cur - 1] = dp[i + 1][cur - 1];
                    else {
                        int val = INT_MAX;
                        for (int next : {1,2,3}) {
                            if (obstacles[i] != next) val = min(val, dp[i+1][next-1]);
                        }
                        dp[i][cur-1] = 1 + val;
                    }
                }
                // printf("(%i,%i): [%i,%i,%i]\n",i, cur, dp[i][0], dp[i][1], dp[i][2]);
            }
