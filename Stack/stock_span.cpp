...
For example, if the prices of the stock in the last four days 
is [7,2,1,2] and the price of the stock today is 2, then the 
span of today is 4 because starting from today, the price of 
the stock was less than or equal 2 for 4 consecutive days.
Also, if the prices of the stock in the last four days 
is [7,34,1,2] and the price of the stock today is 8, 
...
public:
    ...
    int next(int price) {
        int ret = 1;    // for the very first price
        if (!log.empty()) {
            while (!log.empty() && price >= log.top().first) log.pop();
            if (log.empty()) ret = ix + 1;
            else ret = ix - log.top().second;
        }
        log.push({price, ix++});
        return ret;
    }
