...
new BSTIterator([7, 3, 15, null, null, 9, 20]);
...
Follow up:
Could you implement next() and hasNext() to run 
in average O(1) time and use O(h) memory, 
...
private:
    ...
    void Morrisize(TreeNode* node)
    {
        ...
        while (cur->left) {
            ...
            pred->right=cur;    // Morris loop-back
            TreeNode* left=cur->left;
            cur->left=nullptr;  // kill cycle
            cur=left;
        }
        it=cur;
    }
